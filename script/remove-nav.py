...
for item in soup.find_all(src=True):
    ...
    if src.startswith('/static/'):
        item['src'] = src.replace('/static/', 'https://jklukas.github.io/mozilla-reports/static/legacy-knowledge-repo/')

for item in soup.find_all(href=True):
    href = item['href'] or ""
    if href.startswith('/static/'):
        item['href'] = href.replace('/static/', 'https://jklukas.github.io/mozilla-reports/static/legacy-knowledge-repo/')
